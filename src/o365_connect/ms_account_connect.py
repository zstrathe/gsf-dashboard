...
class MSAccount(object):
    ...
    def __init__(self, auth_manual=False):
        ...
        self.site_libs = {
            site_name: self.account_connection.sharepoint()
            .get_site(site_id)
    ...
    def load_account_user(self, auth_manual):
        ...
        token_backend = FileSystemTokenBackend(
            token_path="./settings/auth_data/", token_filename="ms_auth.token"
        )
        account_create = Account(
            credentials=(
                self.connection_settings["client_id"],
                self.connection_settings["client_secret"],
            ),
        ...
        if auth_manual:
            ...
            if (
                account_create.authenticate()
            ):  # will return True if successful authentication, False if it failed
        ...
        try:
            ...
            raise Exception(
                'Account is not authenticated! Manually run "MSAccount(auth_manual=True)" to complete Microsoft authentication and store token for re-use!'
            )
    ...
    def load_account_with_cert(self):
        ...
        tenant, client_id, thumbprint, cert_path = load_setting(
            "sharepoint_cert_credentials"
        ).values()
        key = open(cert_path, "r").read()
        jwt = create_jwt_assertion(
            private_key=key,
            tenant_id=tenant,
        ...
        account = Account(
            (client_id, jwt), auth_flow_type="certificate", tenant_id=tenant
        )
        if not account.authenticate(
            scopes=[f'https://{tenant.split(".")[0]}.sharepoint.com/.default']
        ):
    ...
    def interactive_view_sharepoint_data(self):
        ...
        while stop_flag is False:
            if (
                user_choice := input(
                    '\nEnter folder (using ID) to list files, or "q" to quit: '
                )
            ).lower() == "q":
                return None
            try:
...
class M365ExcelFileHandler:
    def __init__(
        ...
        data_get_method: str = "DL",
        concat_sheets: bool = False,
        load_sheet_kwargs: dict = {},
    ):
        """
        ...
        if self._file_obj is None:
            ...
            self._wb = WorkBook(
                self._file_obj
            )  # get file as WorkBook obj from O365 module, for using API calls specific to excel files

        # Get the valid worksheet names from file
        self._worksheet_names_valid = [
            str(s).replace("Worksheet: ", "")
            for idx, s in enumerate(self._wb.get_worksheets())
        ]

        self._worksheet_names_load = (
            []
        )  # init empty list to store worksheet names to load
        if len(include_sheets) > 0 and len(ignore_sheets) > 0:
            raise Exception(
                'Either "include_sheets" or "ignore_sheets" parameter can be used, but not both!'
            )
        elif len(include_sheets) > 0:
            # case when include_sheets specified
            print(
                f'loading sheet{"s" if len(include_sheets) > 1 else ""}: {str(*include_sheets)}',
                end="...",
                flush=True,
            )
            [
                self._worksheet_names_load.append(sheet)
                if sheet in self._worksheet_names_valid
                else print(
                    f'ERROR: worksheet "{sheet}" is not contained in file: {file_object_id}. Skipping...'
                )
                for sheet in include_sheets
            ]
        elif len(ignore_sheets) > 0:
            # case when ignore_sheets specified
            ## KEEP COMMA AFTER UNPACKED LIST IN F-STRING!! REMOVING IT WILL RESULT IN A SYNTAX ERROR
            print(
                f'ignoring sheet{"s" if len(ignore_sheets) > 1 else ""}: {*ignore_sheets,}',
                end="...",
                flush=True,
            )
            # populate a list of all sheets to load, remove the sheets to ignore in next lines
            [
                self._worksheet_names_load.append(sheet)
                for sheet in self._worksheet_names_valid
            ]
        ...
        if load_data and data_get_method == "DL":
            dl_file_path = self._account.download_sharepoint_file_by_id(
                self._file_id, to_path=Path(f"data_sources/tmp/")
            )
            if dl_file_path:  # will be None if error downloading
                self._downloaded_ExcelFile = pd.ExcelFile(dl_file_path.as_posix())
                # delete file after it's loaded
                dl_file_path.unlink()
            else:
        ...
        self.sheets_data = {
            sname: self.get_sheet_data(
                sname, load_data, data_get_method, **load_sheet_kwargs
            )
            for sname in self._worksheet_names_load
        }

        # concatenate sheets into a single dataframe if concat_sheets == True and load_data == True
        if load_data and concat_sheets:
            self.concat_df = functools.reduce(
                lambda df1, df2: pd.concat([df1, df2], ignore_index=True),
                [sheet.df for sheet in self.sheets_data.values()],
            )
        else:
            self.concat_df = None
    ...
    def get_sheet_data(
        self, sheet_name: str, load_data: bool, data_get_method: str, **kwargs
    ):
        """
        Params:
        ...
        (
            ...
            sheet.min_row,
            sheet.min_col,
        ) = self._sheet_get_used_range_fixed(sheet)
        if load_data:
            if data_get_method.upper() == "DL":
                ...
                raise Exception(
                    f'Invalid "get_method" specified for loading sheet: {sheet_name}!'
                )
        else:
            # set df attribute to None if not loading the sheet data
            sheet.df = None
        return sheet

    def _load_excel_sheet_to_df_download(
        self, sheet: WorkSheet, **kwargs
    ) -> pd.DataFrame:
        df = self._downloaded_ExcelFile.parse(sheet.name, **kwargs)
        print(f"loaded df for {sheet.name}...")
        return df

    def _load_excel_sheet_to_df_api(
        self, sheet: WorkSheet, start_row: int | None = None, end_row: int | None = None
    ) -> pd.DataFrame:
        """
        ...
        if sheet.used_range is not None:
            _column_headers = sheet.get_range(
                f"{sheet.min_col}{sheet.min_row}:{sheet.max_col}{sheet.min_row}"
            ).values[0]
            sheet.column_to_name_map = {
                self._convert_numeric_col_to_alphabetic(idx): col_name
                for (idx, col_name) in enumerate(_column_headers, start=1)
                if col_name != ""
            }
        ...
        row_query_limit = (
            3500000 / len(list(sheet.column_to_name_map.keys()))
        ).__floor__()

        # list to store data values, in case multiple queries need to be combined together
        data_vals = []
        if end_row > row_query_limit:
            query_ranges = [
                (i, i + row_query_limit - 1)
                if i + row_query_limit - 1 < end_row
                else (i, end_row)
                for i in range(start_row, end_row + 1, row_query_limit)
            ]
            ...
            data_vals.extend(
                sheet.get_range(
                    f"{sheet.min_col}{start_row}:{sheet.max_col}{end_row}"
                ).values
            )
        ...
        if not len(df) > 25:
            ...
            for row_idx in range(1, 26):
                for col_idx, cell in enumerate(df.iloc[row_idx, :]):
                    if col_idx in non_dt_cols:
                        # pass on subsequent rows if a non-datetime column has been identified on a prior checked row
                        pass
                    # check if cell is an integer, then check if it can be converted into valid date with year being between 2017 and 2055
                    if isinstance(cell, int):
                        try:
                            if pd.to_datetime(
                                cell, unit="d", origin="1899-12-30"
                            ).year in (range(2017, 2056)):
            ...
            for col in dt_cols:
                ...
                df.iloc[:, col] = pd.to_datetime(
                    df.iloc[:, col], unit="d", origin="1899-12-30", errors="coerce"
                )
    ...
    def _sheet_get_used_range_fixed(self, sheet: WorkSheet):
        ...
        full_range_str = sheet.range_constructor(
            parent=sheet, **{sheet._cloud_data_key: response.json()}
        ).address
        # print('Loading', full_range_str)
        trunc_range_str = full_range_str.split("!")[-1]

        # get the sheet coordinate start (i.e., for "A1:Z335", would extract "A1")
        min_range_str = trunc_range_str.split(":")[0]
        # returns as a list, get first item, should be the only one
        min_row = re.findall(r"\d+", min_range_str)[0]
        min_col = re.findall(r"[a-zA-z]+", min_range_str)[0]

        # get the sheet coordinate extents (i.e., for "A1:Z335", would extract "Z335")
        max_range_str = trunc_range_str.split(":")[-1]
        # returns as a list, get first item, should be the only one
        max_row = re.findall(r"\d+", max_range_str)[0]
    ...
    def _find_last_row_brute_method(self, file_object_id: str, sheet: WorkSheet):
        ...
        *** THIS METHOD IS SLOW DUE TO REPEATED API REQUESTS FOR EACH SHEET, USE AS A LAST RESORT ONLY (should only be necessary for massive sheets that otherwise break the API) ***
        ...
        def _load_json(file_object_id: str, sheet_name: str):
            if os.path.exists(
                file_path := os.path.join(
                    "settings/json_data", f"{file_object_id}.json"
                )
            ):
        ...
        def _write_json(file_object_id: str, sheet_name: str, out_data: dict):
            with open(
                os.path.join("settings/json_data", f"{file_object_id}.json"), "w"
            ) as outfile:
        ...
        # search with 500 columns
        col_search_bound = self._convert_numeric_col_to_alphabetic(500)
        # var to track the lowest column that contains data
        col_low_tracker = 0
        # var to keep track of the highest column that contains data
        col_high_tracker = 0
        ...
        while True:
            ...
            row_search_high = [
                1048576 if row_search_high > 1048576 else row_search_high
            ][0]
            ...
            # flag to track whether the row is empty
            empty_row = True
            for idx, cell in enumerate(check_row, start=1):
                if cell != "":
                    ...
                    # if empty rows have been found, then only jump ahead by the diff between high and low vals
                        ...
                        row_search_high = (
                            (row_search_high - row_search_low) / 2
                        ).__ceil__() + row_search_high
            ...
            if empty_row:
                ...
                if row_search_high - row_search_low < 1000:
                    final_check_bound = (
                        f"A{row_search_low}:{col_search_bound}{row_search_high}"
                    )
                    ...
                    for row_idx, row in enumerate(final_check_query, start=1):
                        for val_idx, val in enumerate(row, start=1):
                            if val != "":
                                ...
                                final_empty_row_count = 0
                                # immediately break and begin checking next row if any nonempty val found
                                break
                                ...
                                if final_empty_row_count >= 5:  #
                                    ...
                                    min_col = self._convert_numeric_col_to_alphabetic(
                                        col_low_tracker
                                    )
                                    max_col = self._convert_numeric_col_to_alphabetic(
                                        col_high_tracker
                                    )
                                    _write_json(
                                        file_object_id, sheet.name, {"max_row": max_row}
                                    )
                                    return (
                                        f"{min_col}1:{max_col}{max_row}",
                                        max_row,
                                        max_col,
                                    )
                else:
        ...
        For example: starting with 56
                        ...
                        - Then on the next loop, the max divisor (2) is used for finding the next character.
                            ...
                            - Append "B" to left of the existing string "D":
                                    returns "BD"
        """
        # function to convert a 1-26 number (int) into a corresponding alphabetic character
        to_alpha = lambda x: chr(x + 64)
        out_str = ""
        while col_number > 0:
            if col_number > 26:
...
class EmailHandler:
    ...
    def get_latest_email_attachment_from_folder(
        self, folder_name, save_filename, dl_attachment=True
    ):
        """
        ...
        if len(attachments) > 0:
            for a in attachments:
                ...
                if a_type == "item":
                    ...
                    return self.extract_attachment_from_email_file(
                        tmpfile_path, save_filename
                    )
                elif a_type == "file":
