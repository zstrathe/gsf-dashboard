    ...
    for idx, (key, val) in enumerate(setting.copy().items()):
        if ".dtype" in key:
            ...
            match enforced_dtype:
                ...
                case _:
                    print(
                        f"ERROR: Could not enforce dtype: {enforced_dtype} for setting: {enforced_setting}"
                    )
            setting[enforced_setting] = enforced_value
            # delete the setting entry for the .dtype specifier
            del setting[key]
    return setting


def generate_multipage_pdf(
    fig_list, pdf_filename, add_pagenum=True, bbox_inches="tight"
):
...
def redirect_logging_to_file(log_file_directory: Path, log_file_name: str):
    ...
    def decorator(function):
        def func_wrapper(*args, **kwargs):
            ...
            # convert log_file into a string of the file path
            log_file_path = log_file.as_posix()
