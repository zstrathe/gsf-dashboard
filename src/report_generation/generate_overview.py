...
class PondsOverviewPlots:
    def __init__(self, select_date, source_db="gsf_data", run_all=True, save_output=True):
        ...
        if run_all:
            ...
            [
                self.output_filenames.append(x)
                for x in (
                    self.plot_scorecard(output_pdf=True),
                    self.plot_potential_harvests(output_pdf=True),
                    self.plot_epa(output_pdf=True),
                )
            ]
    ...
    def plot_legend(self, fig, ax, legend_data, x_align, y_spacing, y_align=0.9):
        ...
        for row in legend_data:
            ...
            for idx, (label, properties) in enumerate(row["labels"].items()):
                ...
                if (
                    "indicator_fill" in properties
                ):  # plot indicators instead of normal text
                    ...
                    circle_width = 0.07 * ax_maxd / ax_dx
                    circle_height = 0.07 * ax_maxd / ax_dy

                    # plot indicator
                    t = ax.add_patch(
                        Ellipse(
                            (x_align[properties["align"]][0], y_align),
                            circle_width,
                            circle_height,
                            color=properties["indicator_fill"],
                            fill=properties["indicator_fill"],
                            clip_on=False,
                        )
                    )
                    ax.text(
                        x_align[properties["align"]][0],
                        y_align,
                        label,
                        color="white",
                        ha="center",
        ...
        for row_y in data_rows_y_bounds:
            ax.add_patch(
                Rectangle(
                    (
                        data_rows_x_bound[0] - row_box_padding,
                        row_y[0][0] - row_box_padding,
                    ),
                    data_rows_x_bound[1] - data_rows_x_bound[0] + row_box_padding * 2,
                    row_y[0][1] - row_y[0][0] + row_box_padding * 2,
                    linewidth=0,
                    edgecolor=None,
                    facecolor=row_y[1],
                    clip_on=False,
        ...
        ax.add_patch(
            Rectangle(
                (max_coords[0] - box_padding, max_coords[1] - box_padding),
                max_coords[2] - max_coords[0] + box_padding * 2,
                max_coords[3] - max_coords[1] + box_padding * 2,
                linewidth=1,
                edgecolor="black",
                facecolor="none",
                clip_on=False,
    ...
    def plot_scorecard(
        ...
        def plot_each_pond(fig, pond_plot, pond_name, select_date):
            ...
            inner_plot = gridspec.GridSpecFromSubplotSpec(
                5, 3, subplot_spec=pond_plot, wspace=-0.01, hspace=-0.01
            )
            ...
            try:
                pond_last_harvest_str = last_harvest_dates_df.loc[
                    last_harvest_dates_df["PondID"] == pond_name, "last_harvest_date"
                ].iloc[0]
            except KeyError:
            ...
            if pond_active:
                ...
                epa_val, epa_date = (
                    epa_df[
                        (epa_df["PondID"] == pond_name)
                        & (epa_df["Date"] == self.select_date)
                    ]
                    .iloc[0]
                    .loc[["epa_val", "measurement_date_actual"]]
                )
                if pd.isna(epa_val):
                    epa_val = None
                    epa_date = None
                ...
                indicators_data = (
                    indicators_df.loc[indicators_df["PondID"] == pond_name]
                    .copy()
                    .fillna(0)
                    .iloc[0]
                )
                ...
                indicator_dict = {
                    "% Nanno": ["less", 80, "%N", "xkcd:fire engine red"],
                    "Handheld pH": [
                        "out-of-range",
                        [7.8, 8.2],
                        "pH",
                ...
                for idx, (key, val) in enumerate(indicator_dict.items()):
                    try:
                        comparison_value = float(indicators_data[key])
                    except Exception as e:
                        print(f'Error: {e}: with converting indicator item: {key}: {val} into a float!')
                        continue
                    comparison_operator_str = val[0]
                    threshold = val[1]
                    if comparison_operator_str == "greater-equal":
                        if comparison_value >= threshold:
                            pond_indicator_data.append([val[2], val[3]])
                    if comparison_operator_str == "less":
                        if comparison_value < threshold:
                            pond_indicator_data.append([val[2], val[3]])
                    if comparison_operator_str == "out-of-range":
                        if (
                            comparison_value < threshold[0]
                            or comparison_value > threshold[1]
                        ):
                            pond_indicator_data.append([val[2], val[3]])
            ...
            if pond_active:
                ...
                title_ax.text(
                    0.5,
                    0.5,
                    f"last harvest/split: {pond_last_harvest_str}",
                    ha="center",
                    va="center",
                )
            ...
            if pond_active:
                ...
                pond_data_harvestable_mass = pond_data[
                    "harvestable_mass_nanno_corrected"
                ]
                if (
                    pd.isna(pond_data_harvestable_mass)
                    or pond_data_harvestable_mass == 0
                ):
                ...
                if (
                    pd.isna(pond_data_harvestable_depth)
                    or pond_data_harvestable_depth == 0
                ):
                ...
                data_plot_dict = {
                    "Measurement": {"Depth:": pond_data_depth, "AFDW:": pond_data_afdw},
                    "Growth": {
                        "5 Days:": running_avg_growth_5d,
                        "14 Days:": running_avg_growth_14d,
                    },
                    "Harvestable": {
                        "Mass:": pond_data_harvestable_mass,
                        "Inches:": pond_data_harvestable_depth,
                    },
                }
            ...
            for idx in range(3):
                ...
                if pond_active:
                    ...
                    text_y = 0.83
                    sub_heading = ax.text(
                        0.5, text_y, subplot_dict_key, ha="center", va="center"
                    )  # subplot heading text
                    ...
                    bb = sub_heading.get_window_extent(
                        renderer=fig.canvas.get_renderer()
                    ).transformed(ax.transAxes.inverted())
                    ax.annotate(
                        "",
                        xy=(bb.x0 - 0.02, bb.y0),
                        xytext=(bb.x1 + 0.025, bb.y0),
                        xycoords="axes fraction",
                        arrowprops=dict(arrowstyle="-", color="k"),
                    )
                    text_y -= 0.19
                    for idx2, (key, val) in enumerate(
                        data_plot_dict[subplot_dict_key].items()
                    ):
                        # subplot item heading
                        ax.text(0.5, text_y, key, ha="center", va="center")
                        text_y -= 0.14
                        ax.text(
                            0.5, text_y, val, ha="center", va="center", weight="bold"
                        )
                        text_y -= 0.2
        ...
        epa_df = query_data_table_by_date_range(
            db_name_or_engine=self.source_db,
            table_name="epa_data",
        ...
        indicators_df = query_data_table_by_date_range(
            db_name_or_engine=self.source_db,
        ...
        measurements_df = query_data_table_by_date_range(
            db_name_or_engine=self.source_db,
            table_name="ponds_data",
        ...
        _tmp_calculated_query_df = query_data_table_by_date_range(
            db_name_or_engine=self.source_db,
        ...
        last_harvest_dates_df = query_data_table_by_date_range(
            db_name_or_engine=self.source_db,
            table_name="ponds_data_calculated",
            query_date_start=self.select_date,
            query_date_end=self.select_date,
        ...
        last_harvest_dates_df["last_harvest_date"] = last_harvest_dates_df[
            "days_since_harvested_split"
        ].apply(
            lambda x: (self.select_date - pd.Timedelta(days=x)).strftime("%-m-%-d-%y")
            if not pd.isna(x)
            else "n/a"
        )
        ...
        for pond_id in measurements_df["PondID"].unique():
            curr_date_mask = (measurements_df["PondID"] == pond_id) & (
                measurements_df["Date"] == self.select_date
            )
            prev_date_mask = (measurements_df["PondID"] == pond_id) & (
                measurements_df["Date"] == self.select_date - pd.Timedelta(days=1)
            )
            active_status = measurements_df.loc[curr_date_mask]["active_status"].iloc[0]
            if active_status:
                curr_depth, curr_afdw = (
                    measurements_df.loc[curr_date_mask][["Depth", "Filter AFDW"]]
                    .iloc[0]
                    .values
                )
                ...
                if pd.isna(curr_afdw) or curr_afdw <= 0:
                    prev_afdw = measurements_df.loc[prev_date_mask]["Filter AFDW"].iloc[
                        0
                    ]
                    if not pd.isna(prev_afdw) or (
                        not pd.isna(prev_afdw) and prev_afdw > 0
                    ):
                        measurements_df.loc[curr_date_mask, "Filter AFDW"] = prev_afdw
                        measurements_df.loc[
                            curr_date_mask, "non_current_data_flag_afdw"
                        ] = True
                    else:
                        measurements_df.loc[
                            curr_date_mask, "non_current_data_flag_afdw"
                ...
                if (pd.isna(curr_depth) or curr_depth <= 0) or (
                    pd.isna(curr_afdw) or curr_afdw <= 0
                ):
                    measurements_df.loc[curr_date_mask, ["Depth", "Filter AFDW"]] = None
                    measurements_df.loc[
                        curr_date_mask,
                        ["non_current_data_flag_depth", "non_current_data_flag_afdw"],
                    ] = False
        ...
        if (
            measurements_df[
                (measurements_df["non_current_data_flag_depth"] == True)
                | (measurements_df["non_current_data_flag_afdw"] == True)
            ].shape[0]
            > 0
        ):
        ...
        [
            title_labels.append([f"BLANK {r}-{c}" for c in range(1, 7)])
        ...
        outer_plots = gridspec.GridSpec(
            len(title_labels), len(title_labels[0]), wspace=0.05, hspace=0.1
        )

        # add title
        title_date = select_date.strftime("%-m/%-d/%Y")
        fig.suptitle(
            f"{plot_title}\n{title_date}", fontweight="bold", fontsize=16, x=0.51, y=0.905, 
        )
        ...
        for idx_plot, pond_plot in enumerate(outer_plots):
            ...
            if "BLANK" not in pond_name:
                ...
                if "BLANK 11-6" in pond_name:
                    status_code_mass_df = measurements_df[
                        ["Date", "PondID", "status_code", "calc_mass_nanno_corrected"]
                    ].copy()

                    total_mass_by_code = (
                        status_code_mass_df.groupby(by="status_code")
                        .agg(
                            pond_count=("status_code", "count"),
                            calc_mass_nanno_corrected=(
                                "calc_mass_nanno_corrected",
                                "sum",
                            ),
                    ...
                    for code in [0, 1, 2, 3, 4, 5, 6]:
                        if code not in total_mass_by_code["status_code"].values:
                            total_mass_by_code = total_mass_by_code.append(
                                {
                                    "status_code": code,
                                    "pond_count": 0,
                                    "calc_mass_nanno_corrected": 0,
                                },
                                ignore_index=True,
                            )

                    total_mass_by_code = total_mass_by_code.set_index(
                        "status_code"
                    ).to_dict(orient="index")

                    for code, vals_dict in total_mass_by_code.copy().items():
                        pond_count, calc_mass = vals_dict.values()
                        total_mass_by_code[code]["pond_count"] = (
                            pond_count
                            if pond_count is not None and pond_count > 0
                            else "-"
                        )
                        total_mass_by_code[code]["calc_mass_nanno_corrected"] = (
                            f"{calc_mass:,.0f} kg"
                    ...
                    legend_data = [
                        {"labels": {"Color key": {"align": "1-l", "weight": "bold"}}},
                        {
                            "labels": {
                        ...
                        },
                        {
                            "labels": {
                                ...
                                "and": {"align": "2"},
                                "any": {"align": "3-r"},
                                total_mass_by_code.get(2).get("pond_count"): {
                                    "align": "4-c",
                                    "excl_color": "Y",
                                },
                                total_mass_by_code.get(2).get(
                                    "calc_mass_nanno_corrected"
                                ): {"align": "5-c", "excl_color": "Y"},
                            },
                            "fill_color": "red",
                        },
                        {
                            "labels": {
                                ...
                                total_mass_by_code.get(3).get("pond_count"): {
                                    "align": "4-c",
                                    "excl_color": "Y",
                                },
                                total_mass_by_code.get(3).get(
                                    "calc_mass_nanno_corrected"
                                ): {"align": "5-c", "excl_color": "Y"},
                            },
                            "fill_color": "tan",
                        },
                        {
                            "labels": {
                                ...
                                "or": {"align": "2"},
                                "2.5% - 2.99%": {"align": "3-r"},
                                total_mass_by_code.get(4).get("pond_count"): {
                                    "align": "4-c",
                                    "excl_color": "Y",
                                },
                                total_mass_by_code.get(4).get(
                                    "calc_mass_nanno_corrected"
                                ): {"align": "5-c", "excl_color": "Y"},
                            },
                            "fill_color": "yellow",
                        },
                        {
                            "labels": {
                                ...
                                "and": {"align": "2"},
                                r"$\geq$ 3.0%": {"align": "3-r"},
                                total_mass_by_code.get(5).get("pond_count"): {
                                    "align": "4-c",
                                    "excl_color": "Y",
                                },
                                total_mass_by_code.get(5).get(
                                    "calc_mass_nanno_corrected"
                                ): {"align": "5-c", "excl_color": "Y"},
                            },
                        ...
                        },
                        {
                            "labels": {
                                ...
                                "and": {"align": "2"},
                                r"$\geq$ 3.0%": {"align": "3-r"},
                                total_mass_by_code.get(6).get("pond_count"): {
                                    "align": "4-c",
                                    "excl_color": "Y",
                                },
                                total_mass_by_code.get(6).get(
                                    "calc_mass_nanno_corrected"
                                ): {"align": "5-c", "excl_color": "Y"},
                            },
                        ...
                        },
                        {
                            "labels": {
                                ...
                                total_mass_by_code.get(1).get("pond_count"): {
                                    "align": "4-c",
                                    "excl_color": "Y",
                                },
                                total_mass_by_code.get(1).get(
                                    "calc_mass_nanno_corrected"
                                ): {"align": "5-c", "excl_color": "Y"},
                            },
                ...
                if (
                    "BLANK 12-6" in pond_name
                ):  # plot the indicator legend in the first blank subplot in row 12
                    legend_data = [
                        {
                            "labels": {
                                "Health/Pest Indicators": {
                                    "align": "1-l",
                                    "weight": "bold",
                                }
                            }
                        ...
                        {
                            "labels": {
                                "Item": {"align": "1-l", "weight": "underline"},
                                "Threshold": {"align": "2", "weight": "underline"},
                                "Symbol": {"align": "3-c", "weight": "underline"},
                            }
                        },
                        {
                            "labels": {
                                ...
                                "%N": {
                                    "align": "3-c",
                                    "indicator_fill": "xkcd:fire engine red",
                                },
                            }
                        },
                        {
                        ...
                        },
                        {
                            "labels": {
                                ...
                                "R": {
                                    "align": "3-c",
                                    "indicator_fill": "xkcd:deep sky blue",
                                },
                            }
                        },
                        {
                            "labels": {
                                "Attached FD111": {"align": "1-l"},
                                r"$\geq$ 1": {"align": "2"},
                                "FD\nA": {
                                    "align": "3-c",
                                    "indicator_fill": "xkcd:poo brown",
                                },
                            }
                        },
                        {
                            "labels": {
                                "Free Floating FD111": {"align": "1-l"},
                                r"$\geq$ 1": {"align": "2"},
                        ...
                        {
                            "labels": {
                                "Golden Flagellates": {"align": "1-l"},
                                r"$\geq$ 1": {"align": "2"},
                                "GF": {"align": "3-c", "indicator_fill": "y"},
                            }
                        },
                        {
                            "labels": {
                                ...
                                "D": {
                                    "align": "3-c",
                                    "indicator_fill": "xkcd:powder pink",
                                },
                            }
                        },
                        {
                            "labels": {
                                "Tetra": {"align": "1-l"},
                                r"$\geq$ 0.5": {"align": "2"},
                                "T": {"align": "3-c", "indicator_fill": "purple"},
                            }
                        },
                        {
                            "labels": {
                                "Green Algae": {"align": "1-l"},
                                r"$\geq$ 0.5": {"align": "2"},
                                "GA": {"align": "3-c", "indicator_fill": "green"},
                            }
                        },
                    ]
                    ...
                    if any_noncurrent_flag:
                        ax.text(
                            0,
                            0.95,
                            "** indicates AFDW or Depth data that is not available for the current day, so using data from previous day",
                            ha="left",
                            va="center",
                            fontsize="small",
                        )

                    total_active_ponds = measurements_df[
                        measurements_df["active_status"] == True
                    ].shape[0]
                    num_active_1_acre = measurements_df[
                        (measurements_df["Column"].isin(["1", "2", "3", "4"]))
                        & (measurements_df["active_status"] == True)
                    ].shape[0]
                    num_active_2_acre = measurements_df[
                        (measurements_df["Column"].isin(["6", "8"]))
                        & (measurements_df["active_status"] == True)
                    ].shape[0]
                    total_active_acres = (num_active_1_acre * 1.1) + (
                        num_active_2_acre * 2.2
                    )
                    ...
                    total_calc_mass = query_data_table_by_date_range(
                        db_name_or_engine=self.source_db,
                        table_name="ponds_data_calculated",
                    ...
                    total_calc_mass["calc_mass_nanno_corrected"] = total_calc_mass[
                        "calc_mass_nanno_corrected"
                    ].replace("_tmp_placeholder_for_ffill", None)
                    total_calc_mass = total_calc_mass.loc[
                        (total_calc_mass["active_status"] == True)
                        & (total_calc_mass["Date"] == self.select_date),
                        "calc_mass_nanno_corrected",
                    ].sum()

                    print_string = (
                        r"$\bf{Total\/\/Active\/\/ponds: }$"
                        + f"{total_active_ponds}\n"
                        + r"$\bf{Active\/\/1.1\/\/acre\/\/ponds: }$"
                        + f"{num_active_1_acre}\n"
                        + r"$\bf{Active\/\/2.2\/\/acre\/\/ponds: }$"
                        + f"{num_active_2_acre}\n"
                        + r"$\bf{Total\/\/Active\/\/Acres: }$"
                        + f"{total_active_acres:,.0f}\n"
                        + r"$\bf{Calculated\/\/total\/\/mass:}$"
                        + f"{total_calc_mass:,.0f} kg"
                    )
                    ax.text(
                        0,
                        0.75,
                    ...
                    growth_data = query_data_table_by_date_range(
                        db_name_or_engine=self.source_db,
                        table_name="ponds_data_aggregate",
                        query_date_start=self.select_date,
                        query_date_end=self.select_date,
                        col_names=[
                            "agg_running_avg_norm_growth_5d",
                            "agg_running_avg_norm_growth_14d",
                        ],
                    ).iloc[0]
                    run_avg_growth_5d = (
                        f'{growth_data["agg_running_avg_norm_growth_5d"]:.3f}'
                        if not pd.isna(growth_data["agg_running_avg_norm_growth_5d"])
                        else "n/a"
                    )
                    run_avg_growth_14d = (
                        f'{growth_data["agg_running_avg_norm_growth_14d"]:.3f}'
                        if not pd.isna(growth_data["agg_running_avg_norm_growth_14d"])
                        else "n/a"
                    )

                    print_string = "Total Growth (g/L/day normalized)"
                    growth_title = ax.text(
                        0,
                        0.75,
                        print_string,
                        ha="left",
                    ...
                    bb = growth_title.get_window_extent(
                        renderer=fig.canvas.get_renderer()
                    ).transformed(ax.transAxes.inverted())
                    ax.annotate(
                        "",
                    ...
                    print_string += (
                        "\n"
                        + r"$\bf{Total\/\/5\/\/Day\/\/Running\/\/Avg\/\/Growth: }$"
                        + run_avg_growth_5d
                        + "\n"
                        + r"$\bf{Total\/\/14\/\/Day\/\/Running\/\/Avg\/\/Growth: }$"
                        + run_avg_growth_14d
                        + "\n"
                    )
                    ...
                    prev_date_processing_data = query_data_table_by_date_range(
                        db_name_or_engine=self.source_db,
        ...
        if output_pdf:
            out_filename = (
                f'./output_files/{plot_title} {select_date.strftime("%Y-%m-%d")}.pdf'
            )
            plt.savefig(out_filename, bbox_inches="tight")
            # close the figure to release it from memory, no longer needed at this stage
            plt.close(fig)
            return out_filename
        else:
            return fig

    def plot_potential_harvests(self, min_afdw=0.50, min_epa=3.0, overall_spacing=0.03, output_pdf:bool=False):
        def gen_fig(title=False):
            ...
            if title:
                title_text1 = (
                    f'Recommended Harvests - {str(select_date.strftime("%-m/%-d/%y"))}'
                )
                title_text2 = (
                    "\nponds with"
                    + r"$\geq$"
                    + f"{min_afdw:.2f} AFDW and"
                    + r"$\geq$"
                    + f'{min_epa:.0f}% EPA, with estimated harvest depth to reach 0.40 AFDW after top off to 13"\n\n'
                ...
                y0 = (
                    t2.get_window_extent(renderer=fig.canvas.get_renderer())
                    .transformed(ax.transAxes.inverted())
                    .y0
                    - overall_spacing
                )
        ...
        def plot_table(
            table_title_id, ax, y_start, data_fontsize=6.4, title_fontsize=8
        ):
            table_title = ax.text(
                0.5,
                y_start,
                f"Column 0{table_title_id}",
                ha="center",
                va="top",
                fontsize=title_fontsize,
                weight="bold",
            )
            table_title_dims = table_title.get_window_extent(
                renderer=fig.canvas.get_renderer()
            ).transformed(ax.transAxes.inverted())

            table_y_max = (
                table_title_dims.y0 - 0.0025
            )  # add 0.0025 spacing (in terms of ax y-coordinate) between the title and top of table
            ...
            temp_table_dims = temp_table.get_window_extent(
                renderer=fig.canvas.get_renderer()
            ).transformed(ax.transAxes.inverted())
            temp_table.remove()
            ...
            table = ax.table(
                ...
                cellLoc="center",
                colColours=["whitesmoke"] * len(df.columns),
                bbox=[
                    temp_table_dims.x0,
                    table_y_max - temp_table_dims.height,
                    temp_table_dims.width,
                    temp_table_dims.height,
                ],
            )
            table.auto_set_font_size(False)
            table.set_fontsize(data_fontsize)
            table.auto_set_column_width(col=list(range(len(df.columns))))
            table_dims = table.get_window_extent(
                renderer=fig.canvas.get_renderer()
            ).transformed(ax.transAxes.inverted())
        ...
        df1 = potential_harvests_df = query_data_table_by_date_range(
            db_name_or_engine=self.source_db,
            table_name="ponds_data",
            query_date_start=self.select_date,
            query_date_end=self.select_date,
        ...
        df2 = query_data_table_by_date_range(
            db_name_or_engine=self.source_db,
            table_name="ponds_data_calculated",
            query_date_start=self.select_date,
            query_date_end=self.select_date,
        ...
        df3 = query_data_table_by_date_range(
            db_name_or_engine=self.source_db,
            table_name="epa_data",
            query_date_start=self.select_date,
            query_date_end=self.select_date,
        ...
        # Combine dfs
        potential_harvests_df = functools.reduce(
            lambda df1, df2: pd.merge(df1, df2, on=["Date", "PondID"], how="outer"),
            [df1, df2, df3],
        )

        # filter by min_afdw and min_epa
        potential_harvests_df = potential_harvests_df[
            (potential_harvests_df["Filter AFDW"] >= min_afdw)
            & (potential_harvests_df["epa_val"] >= min_epa)
        ]

        # drop empty rows (rows with no 'harvestable_mass')
        potential_harvests_df = potential_harvests_df[
            (potential_harvests_df["harvestable_mass_nanno_corrected"] != 0)
            & (~pd.isna(potential_harvests_df["harvestable_mass_nanno_corrected"]))
        ]

        # calculate "Drop To" (current pond level less the "harvestable_depth_inches")
        potential_harvests_df["Drop To"] = (
            potential_harvests_df["Depth"]
            - potential_harvests_df["harvestable_depth_inches"]
        )

        # sort by Column id (ascending) then by 'days since harvested' (desc) then by harvestable mass (desc)
        potential_harvests_df = potential_harvests_df.sort_values(
            by=[
                "Column",
                "days_since_harvested_split",
                "harvestable_mass_nanno_corrected",
        ...
        if len(h_cols) > 0:
            for col_id in h_cols:
                ...
                potential_harvests_df.loc[
                    mask, "Running Total Gallons"
                ] = potential_harvests_df.loc[mask, "harvestable_gallons"].cumsum()
                potential_harvests_df.loc[
                    mask, "Running Total Mass"
                ] = potential_harvests_df.loc[
                    mask, "harvestable_mass_nanno_corrected"
                ].cumsum()
        ...
        potential_harvests_df = potential_harvests_df.rename(
            columns={
                "PondID": "Pond ID",
        ...
        potential_harvests_df = potential_harvests_df[
            [
                ...
                "Pond ID",
                "Drop To",
                "Days Since Harvested/Split",
                "Harvestable Inches",
                "Harvestable Gallons",
                "Harvestable Mass",
                "Running Total Gallons",
                "Running Total Mass",
            ]
        ]
        ...
        potential_harvests_df["Drop To"] = potential_harvests_df["Drop To"].apply(
            lambda x: f'{x}"'
        )
        potential_harvests_df["Days Since Harvested/Split"] = potential_harvests_df[
            "Days Since Harvested/Split"
        ].apply(lambda x: f"{x} days")
        potential_harvests_df["Harvestable Inches"] = potential_harvests_df[
            "Harvestable Inches"
        ].apply(lambda x: f'{x}"')
        potential_harvests_df["Harvestable Gallons"] = potential_harvests_df[
            "Harvestable Gallons"
        ].apply(lambda x: f"{x:,.0f} gal")
        potential_harvests_df["Harvestable Mass"] = potential_harvests_df[
            "Harvestable Mass"
        ].apply(lambda x: f"{x:,.0f} kg")
        potential_harvests_df["Running Total Gallons"] = potential_harvests_df[
            "Running Total Gallons"
        ].apply(lambda x: f"{x:,.0f} gal")
        potential_harvests_df["Running Total Mass"] = potential_harvests_df[
            "Running Total Mass"
        ].apply(lambda x: f"{x:,.0f} kg")
        ...
        while tables_list:
            ...
            df = potential_harvests_df[
                potential_harvests_df["Column"] == column_id
            ].drop("Column", axis=1)
        ...
        if output_pdf:
            filename = (
                f'./output_files/Potential Harvests {select_date.strftime("%Y-%m-%d")}.pdf'
            )
            out_filename = generate_multipage_pdf(
                fig_list, filename, add_pagenum=True, bbox_inches=None
            )
            return out_filename
        else:
            assert len(fig_list) == 1
            return fig_list[0]

    def plot_epa(self, plot_title="Pond EPA Overview", output_pdf:bool=False):
        def _load_epa_dict():
            epa_df = query_data_table_by_date_range(
                db_name_or_engine=self.source_db,
                table_name="epa_data",
            ...
            for pond_id in epa_df["PondID"].unique():
                ...
                for row in pond_df[::-1].iterrows():
                    ...
                    if row_data["epa_actual_measurement"] == True:
                        pond_epa_dict[row_data["Date"].to_pydatetime()] = row_data[
                            "epa_val"
                        ]
        ...
        def plot_each_pond(fig, pond_plot, pond_name):
            inner_plot = gridspec.GridSpecFromSubplotSpec(
                5, 3, subplot_spec=pond_plot, wspace=0, hspace=0
            )
            ...
            title_ax.text(
                0.5, 0.1, title_str, ha="center", va="bottom", fontsize="large"
            )
            ...
            if pond_active:
                ...
                if len(single_pond_data) > 0:
                    ...
                    epa_ax.text(
                        0.5, 0.5, "Active but no data", ha="center", va="center"
                    )
            ...
            if pond_active:
                ...
                if len(single_pond_data) != 0:
                    ...
                    if len(single_pond_data) == 2:
                        epa_pct_chg, delta_days, epa_pct_color = calc_epa_pct_chg(
                            single_pond_data[0], single_pond_data[1]
                        )
                        text_formatted1 = (
                            f'Change ({delta_days} day{"s" if delta_days > 1 else ""}):'
                        )
                        epa_pct_ax.text(
                            0.5,
                            0.7,
                        ...
                        epa_pct_chg1, delta_days1, epa_pct_color1 = calc_epa_pct_chg(
                            single_pond_data[0], single_pond_data[1]
                        )
                        epa_pct_chg2, delta_days2, epa_pct_color2 = calc_epa_pct_chg(
                            single_pond_data[0], single_pond_data[2]
                        )
                        ...
                        epa_pct_ax.text(
                            0.5, 0.3, "n/a", ha="center", va="center", fontsize="large"
                        )
        ...
        # load active_status
        active_status_df = query_data_table_by_date_range(
            db_name_or_engine=self.source_db,
            table_name="ponds_data_calculated",
            query_date_start=self.select_date,
            query_date_end=self.select_date,
        ...
        _active_status_dict = {
            row["PondID"]: row["active_status"]
            for row in active_status_df.to_dict(orient="records")
        }
        ...
        outer_plots = gridspec.GridSpec(
            len(title_labels), len(title_labels[0]), wspace=0.05, hspace=0.2
        )
        ...
        for idx_plot, pond_plot in enumerate(outer_plots):
            ...
            if "BLANK" not in pond_name:
                ...
                ax.axis("off")
                if (
                    "BLANK 11-6" in pond_name
                ):  # plot the color key in the first blank subplot
                    legend_data = [
                        {
                            "labels": {
                                "Color key (latest measurement)": {
                                    "align": "1",
                                    "weight": "bold",
                                }
                            }
                        ...
                        {
                            "labels": {"0% - 1.99%".center(50): {"align": "1"}},
                            "fill_color": "red",
                        },
                        {
                            "labels": {"2% - 2.99%": {"align": "1"}},
                            "fill_color": "yellow",
                        },
                        {
                            "labels": {"3% and up": {"align": "1"}},
                            "fill_color": "mediumspringgreen",
                        },
                        {
                            "labels": {"No EPA data": {"align": "1"}},
                            "fill_color": "lightgrey",
                        },
                    ]
                    x_align = {"1": [1, "center"]}
                    ...
                    self.plot_legend(
                        fig, ax, legend_data, x_align, y_spacing=0.15, y_align=0.8
                    )
        ...
        fig.suptitle(
            f'{plot_title} - {self.select_date.strftime("%-m/%-d/%Y")}',
            fontweight="bold",
            fontsize=16,
            y=0.905,
            va="bottom",
        ...
        fig.text(
            0.75,
            0.905,
            f'\nLatest sample date: {latest_sample_date.strftime("%-m/%-d/%y")}',
