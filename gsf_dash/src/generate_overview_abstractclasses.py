...
class BaseReportProperties:
    ...
    def report_date(self) -> datetime:
        if not hasattr(self, "_report_date"):
            raise Exception(
                f'ERROR! Could not create instance of {self.__name__} because no "report_date" was provided!'
            )
    ...
    def report_date(self, report_date_: datetime) -> None:
        if not isinstance(report_date_, datetime):
            raise Exception(
                f"ERROR! Could not create instance of {self.__name__} because an invalid report_date was provided! It should be a datetime.datetime value!"
            )
...
class BasePondsGridReport(ABC, BaseReportProperties):
    ...
    def run(self) -> str | None:
        ...
        plot_grid = gridspec.GridSpec(
            nrows=max(N_ROWS_SMALL, N_ROWS_LARGE) + N_EMPTY_ROWS_BOTTOM,
            ncols=N_COLS_SMALL + N_COLS_LARGE,
            figure=self.fig,
            left=0.06,
            right=0.94,
            top=0.93,
        ...
        if self.save_output:
            ...
            # reduce DPI by scale factor... should result in output file actual size being accurate to A4 sheet size?
    ...
    def plot_legend(self, fig, ax, legend_data, x_align, y_spacing, y_align=0.9):
        ...
        for row in legend_data:
            ...
            for idx, (label, properties) in enumerate(row["labels"].items()):
                ...
                if "indicator_fill" in properties:
    ...
    @abstractmethod
    def plot_each_pond(
        self, subplot_spec: gridspec.SubplotSpec, pond_id: str, n_rows: int, n_cols: int
    ):
        """
        Ex:
...
class TestReport(BasePondsGridReport):
    ...
    def plot_each_pond(self, subplot_spec, pond_id, n_rows=5, n_cols=3):
        if "BLANK" in pond_id:
            ...
            # slightly negative vert/horiz padding to eliminate spaces in subplot
...
class ExpenseGridReport(BasePondsGridReport):
    def __init__(self, report_date: datetime, **kwargs):
        ...
        # sets/overrides base fontsize
        self.fontsize_base = 12
    ...
    def load_data(self) -> None:
        ...
        self.ytd_expense_data = query_data_table_by_date_range(
            db_name_or_engine="gsf_data",
            table_name="ponds_data_expenses",
            query_date_start=self.ytd_start,
            query_date_end=self.report_date,
        )
        self.ytd_harvest_data = query_data_table_by_date_range(
            db_name_or_engine="gsf_data",
        ...
        self.mtd_expense_data = self.ytd_expense_data[
            self.ytd_expense_data["Date"] >= self.mtd_start.strftime("%Y-%m-%d")
        ]
        self.mtd_harvest_data = self.ytd_harvest_data[
            self.ytd_harvest_data["Date"] >= self.mtd_start.strftime("%Y-%m-%d")
        ]
    ...
    def plot_each_pond(self, subplot_spec, pond_id, n_rows=5, n_cols=3):
        if "BLANK" in pond_id:
            ...
            title_ax.text(
                ...
                ha="center",
                va="center",
                fontsize=self.fontsizes["default"],
            )
            ...
            pond_mtd_expense = (
                self.mtd_expense_data[self.mtd_expense_data["PondID"] == pond_id]
                .sum(numeric_only=True)
                .sum()
            )
            pond_ytd_expense = (
                self.ytd_expense_data[self.ytd_expense_data["PondID"] == pond_id]
                .sum(numeric_only=True)
                .sum()
            )
            pond_mtd_harvested = (
                self.mtd_harvest_data[self.mtd_harvest_data["PondID"] == pond_id]
                .sum(numeric_only=True)
                .sum()
            )
            pond_ytd_harvested = (
                self.ytd_harvest_data[self.ytd_harvest_data["PondID"] == pond_id]
                .sum(numeric_only=True)
                .sum()
            )
            ...
            if pond_ytd_expense > 0:
                ...
                # right justify data
                mtd_expense_fmt = (
                    f"${int(pond_mtd_expense):,}" if pond_mtd_expense > 0 else "- "
                )
                mtd_harvested_fmt = (
                    f"{int(pond_mtd_harvested):,} kg"
                    if pond_mtd_harvested >= 1
                    else "- "
                )
                ytd_expense_fmt = (
                    f"${int(pond_ytd_expense):,}" if pond_ytd_expense > 0 else "- "
                )
                ytd_harvested_fmt = (
                    f"{int(pond_ytd_harvested):,} kg"
                    if pond_ytd_harvested >= 1
                    else "- "
                )
                data_ax.text(
                    0.9,
    ...
    def plot_annotations(self):
        ...
        self.fig.text(
            0.7,
            0.95,
            f'YTD Period Beginning: {self.ytd_start.strftime("%B %Y")}',
            ha="left",
            va="top",
            fontsize=self.fontsizes["default"],
        )
        ...
        ytd_co2_cost_data = query_data_table_by_date_range(
            db_name_or_engine="gsf_data",
            table_name="co2_usage",
            query_date_start=self.ytd_start,
            query_date_end=self.report_date,
            col_names=["total_co2_cost"],
        )
        mtd_co2_cost_data = ytd_co2_cost_data[
            ytd_co2_cost_data["Date"] >= self.mtd_start.strftime("%Y-%m-%d")
        ]
        ...
        mtd_expense_total = (
            self.mtd_expense_data.sum(numeric_only=True).sum() + mtd_co2_cost
        )
        ytd_expense_total = (
            self.ytd_expense_data.sum(numeric_only=True).sum() + ytd_co2_cost
        )
        # left justify categories
        self.fig.text(
            0.07,
            0.14,
            f"MTD Total Expenses:\nYTD Total Expenses:",
            ha="left",
            va="top",
            ma="left",
            weight="bold",
        ...
        self.fig.text(
            0.23,
            0.14,
            f"${int(mtd_expense_total):,}\n${int(ytd_expense_total):,}",
            ha="left",
            va="top",
            ma="right",
            fontsize=self.fontsizes["medium"],
        ...
        self.fig.text(
            0.33,
            0.14,
            f"MTD Total *Calculated Harvested:\nYTD Total *Calculated Harvested:",
            ha="left",
            va="top",
            ma="left",
            weight="bold",
        ...
        self.fig.text(
            0.575,
            0.14,
            f"{int(mtd_harvested_total):,} kg\n{int(ytd_harvested_total):,} kg",
            ha="left",
            va="top",
            ma="right",
            fontsize=self.fontsizes["medium"],
        ...
        # right justify data
        mtd_avg_cost = (
            f"${mtd_expense_total/mtd_harvested_total:.2f} /kg"
            if mtd_harvested_total > 0
            else "-   "
        )
        ytd_avg_cost = (
            f"${ytd_expense_total/ytd_harvested_total:.2f} /kg"
            if ytd_harvested_total > 0
            else "-   "
        )
        self.fig.text(
            0.79,
        ...
        self.fig.text(
            0.07,
            0.11,
            "*Calculated harvest mass is based on pond 'depth', 'density', and '% nanno' measurements to estimate change in mass between the day of harvest the next day with available data",
            ha="left",
            va="top",
            fontsize=self.fontsizes["small"],
        )
...
class BaseTableReport(ABC, BaseReportProperties):
    def __init__(
        self, report_date: datetime, report_subtitle: str | None = None, **kwargs
    ):
        self.report_date = report_date
        # OPTIONAL formatted string for adding subtitle/additional information below the report title
    ...
    def run(self) -> str:
        ...
        def gen_fig(title_page=False) -> (plt.figure, plt.axes, float):
            """
            params:
                title_page: bool:
            ...
            if title_page:
                ...
                y0 = (
                    t1.get_window_extent(renderer=fig.canvas.get_renderer())
                    .transformed(ax.transAxes.inverted())
                    .y0
                    - self.inter_table_spacing
                )
            else:
                y0 = 1
            return fig, ax, y0
        ...
        def plot_table(
            table_title, df, ax, y_start, data_fontsize=6.4, title_fontsize=8
        ):
            table_title = ax.text(
                0.5,
                y_start,
        ...
        for curr_table in self.data_dict_list:
            ...
            y_align = plot_table(
                table_title=curr_table.get("title"),
                df=curr_table.get("df"),
                ax=ax,
                y_start=y_align,
            )
        ...
        # append the last/most-recent figure to the fig list before generating output file
        fig_list.append(fig)
...
class PotentialHarvestsReport:
    ...
    def plot_potential_harvests(self, overall_spacing=0.03):
        ...
        if len(potential_harvests_dict.get("data", {})) > 0:
            potential_harvests_dict["columns"] = list(
                potential_harvests_dict["data"][
                    list(potential_harvests_dict["data"].keys())[0]
                ][0].keys()
            )
        else:
        ...
        def gen_fig(title=False):
            ...
            # margin padding (in inches) between ax and full fig on all sides, just to maintain a margin \
            # for printing without needing to rescale, as well as room for header/footer info to be added (page numbers, etc.)
            ax_border_pad = 0.5
            ax_width = (plt_width - ax_border_pad * 2) / plt_width
            ax_height = (plt_height - ax_border_pad * 2) / plt_height
            ax_left_x = (1 - ax_width) / 2
            ...
            if title:
                title_text1 = (
                    f'Potential Harvests - {str(select_date.strftime("%-m/%-d/%y"))}'
                )
                title_text2 = (
                    "\nponds with"
                    + r"$\geq$"
                    + "0.50 AFDW and"
                    + r"$\geq$"
                    + '3% EPA, with estimated harvest depth to reach 0.40 AFDW after top off to 13"\n\n'
        ...
        while tables_list:
            # set 'table_title' equal to the key from data
            table_title = tables_list.pop(0)

            # create dataframe from data in dict format
            df = pd.DataFrame(
                potential_harvests_dict["data"][table_title],
                columns=potential_harvests_dict["columns"],
            )
            ...
            if y_align == "start_new_page":
                # show plot for testing in jupyter nb
                plt.show()
                fig_list.append(fig)
                fig, ax, y_align = gen_fig()
                y_align = plot_table(table_title, ax, y_align)
