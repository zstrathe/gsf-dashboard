...
class BaseTimeSeriesGraphReport(ABC):
    def __init__(self, begin_date: datetime, end_date: datetime, run: bool = True, **plot_kwargs):
        if run:
            data_df = self.query_data(begin_date, end_date)
            if 'Date' not in data_df.columns:
                raise Exception('ERROR: could not find a valid "Date" column in data, so can not generate plot!')
    ...
    @abstractmethod
    def query_data(self) -> pd.DataFrame: # , begin_date: datetime, end_date: datetime db_name: str, table_name: str, col_names: list[str]
        pass
        '''
...
class EPAHealthGraph(BaseTimeSeriesGraphReport):
    ...
    def generate_plot(self, data_df: pd.DataFrame) -> Path:
       ...
       # plt.figure(figsize=(plt_width*scale_factor, plt_height*scale_factor))
        fig, (ax1, ax2) = plt.subplots(2, figsize=(plt_width*scale_factor, plt_height*scale_factor))
    ...
    def query_data(self, begin_date: datetime, end_date: datetime) -> pd.DataFrame:
        epa_data_df = query_data_table_by_date_range(db_name_or_engine='gsf_data_test3', table_name='epa_data', query_date_start=begin_date, query_date_end=end_date)
        
        afdw_data_df = query_data_table_by_date_range(db_name_or_engine='gsf_data_test3', table_name='ponds_data', query_date_start=begin_date, query_date_end=end_date, col_names=['Column', 'Filter AFDW'])
        ...
        joined_df['epa_fa_density_ratio'] = joined_df['epa_val_total_fa'] / joined_df['Filter AFDW']
